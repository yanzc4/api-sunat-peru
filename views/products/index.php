...
<body class="min-h-screen bg-[#f4f6f8] text-[#182230] antialiased dark:bg-[#0b0d10] dark:text-[#edf0f3]" style="font-family:'IBM Plex Sans',sans-serif">
    <header class="border-b border-slate-200 bg-white dark:border-white/10 dark:bg-[#111419]">
        <div class="mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-2"><span class="hidden text-sm text-slate-500 sm:inline dark:text-slate-400"><?= $e($nombre) ?></span><button id="theme-toggle" class="admin-icon-button" type="button" aria-label="Cambiar tema">◐</button><a href="/logout" class="admin-icon-button" aria-label="Cerrar sesión">↗</a></div>
        </div>
    ...
    document.querySelectorAll('[data-open-modal]').forEach(b=>b.addEventListener('click',()=>{form.reset();form.elements.id.value='';document.getElementById('productoTitle').textContent='Nuevo producto';alertBox.classList.add('hidden');openModal()}));document.querySelectorAll('[data-close-modal]').forEach(b=>b.addEventListener('click',closeModal));
